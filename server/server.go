...
func (s *Server) startShutdownListener() {
  // todo: 因为服务停止了之后， 服务的节点信息还是会存在consul里面， 时间大概是kv数据的循环写入时间， 所以这里并不是服务停止的时候删除注册信息的逻辑， 目前rpcx的服务停止之后， 会在 consul的 kv 数据里删除服务节点信息是靠 注册的时候设置了 kv数据的超时时间， 然后服务根据超时时间的配置不断循环写入 kv数据，一旦服务停止了， 就会停止这个循环写入， 等kv数据超时之后， consul会自动删除该数据， 所以节点信息就会消失了
  // todo: 注册信息自动消失的关键是， consul kv数据一旦超时了， 怎么设置自动删除
...
func (s *Server) Serve(network, address string) (err error) {
  logs.Debugf("=== call service Serve %+v, %+v ===", network, address)
  s.startShutdownListener() // todo: 监听服务是否shutdown状态
  var ln net.Listener
  ln, err = s.makeListener(network, address)
  if err != nil {
    logs.Debugf(" s.makeListener error -->>> %+v", err)
    return
  }
  ...
  // todo: why here use serveListner??? because you can not return a http daemon server in
  // todo: a func return error, the http2 daemon server can not return, if return, it will quit!
...
// todo: impletement ServeHTTP of *Server
...
func (s *Server) serveByHTTP2(address string) {
  logs.Debugf("=== call serveByHTTP2 ===")
  h2s := &http2.Server{}
  handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
    s.handleHTTP2Request(w, r)
  })
  server := &http.Server{
    Addr:    address,
    //Addr:    "8972",
